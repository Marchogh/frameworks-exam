...
export const showAndHideAlert = (title, text, level, delay = 10000) =>
    ...
    try {
      ...
      if (response.status === 401) {
        dispatch(
          showAndHideAlert(
            "Login",
            "You need to login to post Category!",
            "alert"
          )
        );
      } else {
...
export const postBook = book =>
  async function(dispatch) {
    ...
    try {
      ...
      if (response.status === 401) {
        dispatch(
          showAndHideAlert("Login", "You need to login to post Books!", "alert")
        );
      } else {
        await response.json();
        dispatch(loadBooks());
...
export const deleteCategory = deleteId =>
  async function() {
    if (deleteId === "") return;
    try {
